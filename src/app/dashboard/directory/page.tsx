...
function DirectoryContent() {
  const { entrepreneurs, categories, currentUser, connections, savedEntrepreneurs } = useAppContext();
  ...
  const filteredEntrepreneurs = useMemo(() => {
    ...
    if (filterParam === "favourites") {
      result = entrepreneurs.filter(e => savedEntrepreneurs.includes(e.id));
    }
    ...
    return result;
  }, [entrepreneurs, currentUser?.id, connections, searchQuery, selectedCategory, selectedLocation, selectedLookingFor, sortBy, filterParam, savedEntrepreneurs]);
  ...
  const hasActiveFilters = searchQuery || selectedCategory || selectedLocation || selectedLookingFor;

  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div>
        <h2 className="text-2xl font-bold tracking-tight">
          {filterParam === "favourites" ? "Favourite Profiles" : "Find Entrepreneurs"}
        </h2>
        <p className="text-muted-foreground mt-1">
          {filterParam === "favourites" 
            ? "Your saved connections and favourite profiles." 

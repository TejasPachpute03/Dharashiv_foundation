...
export default function DashboardOverview() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div>
        <h2 className="text-3xl font-bold tracking-tight">Good Morning, {currentProfile?.name.split(' ')[0] || "Business"} 👋</h2>
        <p className="text-muted-foreground mt-2">Grow your network. Discover opportunities. Build stronger businesses.</p>
      </div>

      <ProfileCompletionNudge role="business" />
      <ActivityTicker />
      <DashboardActionGrid role="business" />
    </div>
  );
}

...
import { 
  LayoutDashboard, Search, Calendar, Megaphone, Bell, 
  UserCircle, Settings, LogOut, Lightbulb, Briefcase, Users
} from "lucide-react";
...
export function StudentSidebar({ isOpen, setIsOpen }: SidebarProps) {
  ...
  const mainNav = [
    ...
    { href: "/student/discover", label: "Discover", icon: <Search className="w-5 h-5" /> },
  ];

  const studentNav = [
    { href: "/student/network", label: "My Network", icon: <Users className="w-5 h-5" /> },
    { href: "/student/events", label: "Events", icon: <Calendar className="w-5 h-5" /> },
    { href: "/student/announcements", label: "Announcements", icon: <Megaphone className="w-5 h-5" /> },
    { href: "/student/notifications", label: "Notifications", icon: <Bell className="w-5 h-5" /> },
    { href: "/student/idea-submission", label: "Idea Submission", icon: <Lightbulb className="w-5 h-5" /> },
    { href: "/student/jobs", label: "Jobs/Vacancies", icon: <Briefcase className="w-5 h-5" /> },
  ];

  const accountNav = [
    { href: "/student/profile", label: "My Profile", icon: <UserCircle className="w-5 h-5" /> },
  ...
  const NavItem = ({ item }: { item: any }) => {
    ...
    const highlight = isExactDashboard || (item.href !== "/student" && isActive);

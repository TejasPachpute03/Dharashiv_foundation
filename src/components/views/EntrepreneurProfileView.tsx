...
interface EntrepreneurProfileViewProps {
  id: string;
}

export function EntrepreneurProfileView({ id }: EntrepreneurProfileViewProps) {
  ...
  const dashboardPath = getDashboardForRole(currentUser?.role);
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 animate-in fade-in duration-500">
      <div className="flex items-center justify-between">
        <Button variant="ghost" className="mb-2 px-0 hover:bg-transparent" onClick={() => router.back()}>
                    ...
                    return (
                      <div key={conn.id} className="flex items-center space-x-3 p-3 rounded-lg hover:bg-muted/50 transition-colors border">
                        ...
                        <div className="flex-1 min-w-0">
                          <Link href={`${dashboardPath}/entrepreneur/${ent.id}`} className="font-medium text-foreground hover:underline block truncate" onClick={() => setShowConnectionsModal(false)}>
                            {ent.name}
                          </Link>
